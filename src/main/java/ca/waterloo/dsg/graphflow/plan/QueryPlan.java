...
import ca.waterloo.dsg.graphflow.plan.operator.sink.SinkPrint;
...
public class QueryPlan implements Serializable {
    ...
    @Getter public Sink sink;
    //TODO 实现匹配结果输出
    @Setter public SinkType sinkType = SinkType.PRINT;
    ...
    @Getter transient private List<Triple<String /* name */, Long /* i-cost */, Long /* number output tuples */>> operatorMetrics = new ArrayList<>();
    ...
    /**
     * Constructs a {@link QueryPlan} object.
     */
    public QueryPlan(ScanSampling scan) {
     ...
     */
    public QueryPlan(Operator lastOperator) {
     ...
     */
    public QueryPlan(List<Operator> subplans) {
     ...
     */
    public QueryPlan(Scan lastOperator, double estimatedNumOutTuples) {
        this(lastOperator);
    ...
    public void init(Graph graph, KeyStore store) {
        ...
        switch(sinkType) {
            case COPY:
                sink = new SinkCopy(queryGraph, lastOperator.getOutTupleLen());
                break;
            case PRINT:
                sink = new SinkPrint(queryGraph);
                break;
            case LIMIT:
        ...
        subplanLoop : for (int i = 0; i < subplans.size() - 1; i++) {
            ...
            for (var probe : probes) {
                if (probe.getInSubgraph() == build.getProbingSubgraph()) {
                    probe.setHashTables(new HashTable[]{ hashTable });
                    continue subplanLoop;
                }
            }
        }
        for (var subplan : subplans) {
     ...
     */
    public String getOutputLog() throws LimitExceededException {
    ...
    void setStats() {
        ...
        for (int i = 0; i < operatorMetrics.size() - 1; ++i) {
     ...
     */
    public boolean isSameAs(QueryPlan otherQueryPlan) {
     ...
     */
    public QueryPlan shallowCopy() {
        return new QueryPlan(new ArrayList<>(this.subplans));
    }
     ...
     */
    public QueryPlan copy(boolean isThreadSafe) {

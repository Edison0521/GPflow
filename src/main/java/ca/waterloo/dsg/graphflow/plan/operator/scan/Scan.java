...
public class Scan extends Operator implements Serializable {
    ...
    @Getter protected String fromQueryVertex, toQueryVertex; //query graph
    short fromType, toType, labelOrToType; //query graph

    SortedAdjList[] fwdAdjList; //graph
    private int fromVertexStartIdx, fromVertexEndIdx; //graph
    int[] vertexIds; //graph
    short[] vertexTypes; //graph
    ...
    public Scan(QueryGraph outSubgraph) {
        ...
        if (outSubgraph.getQEdges().size() > 1) {
            throw new IllegalArgumentException();
        }
        var queryEdge = outSubgraph.getQEdges().get(0);
        fromType = queryEdge.getFromType();
        toType = queryEdge.getToType();
        labelOrToType = queryEdge.getLabel();
        lastRepeatedVertexIdx = 0;
        fromQueryVertex = queryEdge.getFromQueryVertex();
        toQueryVertex = queryEdge.getToQueryVertex();

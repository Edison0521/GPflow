...
public class gpflow {
    ...
    public static String[] toOptimizer(String InputGraph, String QueryGraph,String pre_Table) throws IOException {
    ...
    public static void main(String[] args) throws IOException, InterruptedException, Operator.LimitExceededException {
        ...
        //图模式在简化图上进行同态匹配
        int RGraph_count = 0;
        File dir_path = new File("test/querys");
        ...
        for(int i=0;i<RGraph_count;i++) {
            ...
            String GraphVertices = "test/origin/node.csv";
            String GraphEdges = "test/origin/edge.csv";
            String InputGraph = "test/hadel";
            String QueryGraph = "test/querys/patterns"+i+".txt";
        ...
        BufferedReader bufferedReader = new BufferedReader(new InputStreamReader(new FileInputStream("result/runningTime.txt")));
        double preCostTime = Double.parseDouble(bufferedReader.readLine());
        bufferedReader.close();
        BufferedWriter bufferedWriter = new BufferedWriter(new OutputStreamWriter(new FileOutputStream("result/runningTime.txt")));

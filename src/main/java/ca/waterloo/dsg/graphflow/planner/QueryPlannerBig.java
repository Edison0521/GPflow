...
public class QueryPlannerBig extends QueryPlanner {
    ...
    public QueryPlan plan() {
        ...
        var bestPlan = subgraphPlans.get(numQVertices).get(0);
        for (var i = 1; i < subgraphPlans.get(numQVertices).size(); i++) {
            var plan = subgraphPlans.get(numQVertices).get(1);
    ...
    private void considerLeastSelectiveScans() {
        ...
        var qEdges = queryGraph.getQEdges();
        ...
        for (var i = 0; i < NUM_TOP_PLANS_KEPT; i++) {
            ...
            outputSubgraph.addQEdge(edgesToScan[i]);
            var scan = new Scan(outputSubgraph);
            var queryPlan = new QueryPlan(scan, numEdgesToScan[i]);
    ...
    private void considerNextQueryExtensions() {
        ...
        var newQueryPlans = new ArrayList<QueryPlan>(NUM_TOP_PLANS_KEPT);

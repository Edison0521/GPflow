...
public class Catalog {
        ...
        for (var ALD : ALDs) {
            ...
            while (numVertices >= 2) {
                ...
                for (var i = 0; i < inSubgraphs.size(); i++) {
                    if (inSubgraphs.get(i).getNumQVertices() != numVertices) {
                        continue;
                    }
                    var it = queryGraph.getSubgraphMappingIterator(inSubgraphs.get(i));
                    var newNumEdgesMatched = inSubgraphs.get(i).getQEdges().size();
    ...
    private void init(Graph graph, KeyStore store, QueryPlan[] queryPlanArr) {
    ...
    /**
     * Executes the {@link QueryPlan}s.
     */
    public void execute(QueryPlan[] queryPlanArr) throws InterruptedException {
    ...
    private void logOutput(Graph graph, QueryPlan[] queryPlanArr) {

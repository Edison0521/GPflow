...
public class ParseTreeVisitor extends GraphflowBaseVisitor<QueryGraph> {
    ...
    public QueryGraph visitMatchPattern(MatchPatternContext ctx) {
        ...
        for (var queryEdge : queryGraph.getQEdges()) {
            queryEdge.setFromType(queryGraph.getQVertexType(queryEdge.getFromQueryVertex()));
            queryEdge.setToType(queryGraph.getQVertexType(queryEdge.getToQueryVertex()));
        }
        return queryGraph;
    }
    ...
    private void visitEdge(EdgeContext ctx, QueryGraph queryGraph) {
        ...
        } else if (queryGraph.getQVertexToTypeMap().containsKey(fromQVertex)) {
            queryEdge.setFromType(queryGraph.getQVertexToTypeMap().get(fromQVertex));
        }
        if (null != ctx.vertex(1).type()) {
            var toType = ctx.vertex(1).type().variable().getText();
            queryEdge.setToType(store.getTypeKeyAsShort(toType));
        } else if (queryGraph.getQVertexToTypeMap().containsKey(toQVertex)) {
            queryEdge.setToType(queryGraph.getQVertexToTypeMap().get(toQVertex));
        }
        if (null != ctx.label()) {
